...
def _cleanup_expired() -> None:
    ...
    with _lock:
        expired = [jid for jid, j in _jobs.items() if j.created_at < cutoff and j.status != "running"]
        for jid in expired:
            del _jobs[jid]
...
def start_case_generation_job(
    ...
    *,
    ...
    generate_case_draft_precheck(db, project=project, user=user, robot_instance=robot_instance)
...
def _run_with_agent(db: Session, job: CaseGenerationJob) -> None:
    ...
    if _kb_enabled and rag_mode != "agentic":
        kb_snippets, similar_ids = _fetch_kb_examples(
            db, project=project, user=user, prompt=job.prompt, limit=_kb_limit,
        )
    ...
    if _kb_enabled and rag_mode != "agentic" and kb_snippets:
        _append_job_log(
            job,
            {
                "kind": "case_gen_log",
                "phase": "kb_local",
                "message": f"Web 侧预检索参考用例 {len(similar_ids)} 条",
                "hits": len(similar_ids),
            },
        )
    ...
    task_id = submit_case_gen_generate(
        ...
        job.prompt,
        kb_snippets if rag_mode != "agentic" else None,
        project_id=project.id,
