...
基于阿里 **[MAI-UI-2B](https://huggingface.co/Tongyi-MAI/MAI-UI-2B)** 的本地 **GUI Grounding** 子项目：输入 APP 截图 + 自然语言描述，输出 UI 元素坐标。与 `autoglm_phone_agent`、`midscene_agent` 并列，**不修改**其他子项目代码。
...
3. 镜像仍失败时，开 VPN 走官方：
   ...
   ```
4. 仍失败：浏览器打开 [hf-mirror 模型页](https://hf-mirror.com/mlx-community/MAI-UI-2B-bf16-v2) 下载到 `mai_ui_agent/models/MAI-UI-2B-bf16-v2/`，在 `.env` 设 `MAI_UI_MODEL` 为该目录绝对路径。
...
正确顺序：**终端 A** 启动服务并等待就绪 → **终端 B** `python -m mai_ui_agent.cli --check`。
...
cd mai_ui_agent
source .venv/bin/activate
pip install -e .    # 注册 mai_ui_agent 包（仅需一次）
```
...
```bash
# 方式 1：进入子项目
cd mai_ui_agent && source .venv/bin/activate
python -m mai_ui_agent.cli --check

# 方式 2：任意目录
bash mai_ui_agent/scripts/run_cli.sh --check
```
...
```bash
python -m mai_ui_agent.cli \
  --image /path/to/screenshot.png \
  --query "登录按钮"

# 批量
python -m mai_ui_agent.cli -i screen.png \
...
```python
from mai_ui_agent import MaiUiGroundingAgent, load_config
...
1. **终端 A**（`mai_ui_agent` 目录）：`bash scripts/serve_grounding_mlx.sh`，等待出现 `Grounding 服务: http://127.0.0.1:8101`。
2. 仓库根 `.env` 建议：
